...
<body x-data="{ page: 'profile', 'loaded': true, 'darkMode': false, 'stickyMenu': false, 'loanTypeModal' : false, 'editLoanTypeModal': false , 'sidebarToggle': false, 'scrollTop': false, 'newFineModal': false, 'editFineModal': false, 'newBonusModal': false, 'editBonusTypeModal': false,  'editStageModal': false}"
                                    ...
                                    <div class="relative ">
                                        ...
                                        <div class="rounded-xl p-6 dark:border-gray-800 border" x-data="{ activeTab: 'loans-type' }">
                                            ...
                                            <div class="pt-4 dark:border-gray-800">
                                                ...
                                                <div x-show="activeTab === 'transactions'" x-data="loanTransactionsTable()">
                                                                            ...
                                                                            <tr>
                                                                                <td colspan="9" class="px-4 py-12 text-center">
                                                                                    <div class="inline-flex flex-col items-center justify-center space-y-4 p-4">
                                                                                        ...
                                                                                        <div class="space-y-2">
                                                                                            <h2 class="text-xl font-semibold text-gray-700">Loading loan transactions...</h2>
                                                                        ...
                                                                        <tbody class="divide-x divide-y divide-gray-200 dark:divide-gray-800">
                                                                            <template x-for="transaction in paginatedTransactions" :key="transaction.transactionId">
                                                                                <tr class="transition hover:bg-gray-50 dark:hover:bg-gray-900">
                                                                                    ...
                                                                                    <td class="p-4 whitespace-nowrap">
                                                                                        <div class="flex items-center col-span-2">
                                                                                            <p class="text-xs font-sm text-gray-700 dark:text-gray-400" x-text="transaction.memberName"></p>
                                                                                        </div>
                                                                                    </td>
                                                                                    <!-- Loan -->
                                                                                    <td class="p-4 whitespace-nowrap">
    ...
    <script>
        ...
        function loanTransactionsTable() {
            return {
                // Data properties
                transactions: [],
                page: 1,
                ...
                // Initialize
                async init() {
                    console.log('Loan transactions table initialized');
                ...
                async loadTransactions() {
                    this.isLoading = true;
                    try {
                        const response = await fetch('/loans/transactions/all');
                        const data = await response.json();

                        if (data.success && data.data) {
                            this.transactions = data.data.map(item => ({
                                transactionId: 'TXN' + String(item.transactionId).padStart(5, '0'),
                                memberName: item.member_name || 'N/A',
                                loanType: item.loan_type_name || 'N/A',
                                interestRate: item.interest_rate ? item.interest_rate + '%' : '0%',
                                borrowed: 'KES ' + (parseFloat(item.borrowed || 0).toLocaleString()),
                                total: 'KES ' + (parseFloat(item.total || 0).toLocaleString()),
                                ...
                                repaid: 'KES ' + (parseFloat(item.repaid || 0).toLocaleString()),
                                status: item.status || 'N/A',
                                // Add raw date for frequency filtering
                                rawDate: item.start_date ? new Date(item.start_date) : new Date()
                            }));
                        }
                    } catch (error) {
                ...
                getStatusClass(status) {
                    const statusMap = {
                        ...
                        'Stopped': 'bg-warning-100 text-warning-600 dark:bg-warning-900/30 dark:text-warning-400',
                        'Repaid': 'bg-success-100 text-success-600 dark:bg-success-900/30 dark:text-success-400',
                ...
                printLoansReport() {
                    const printWindow = window.open('', '_blank');
                    const currentData = this.paginatedTransactions;

                    const tableRows = currentData.map(loan => `
                        <tr style="border-bottom: 1px solid #e5e7eb;">
                            <td style="padding: 12px; text-align: left;">${loan.transactionId}</td>
                            <td style="padding: 12px; text-align: left;">${loan.memberName}</td>
                            <td style="padding: 12px; text-align: left;">${loan.loanType} (${loan.interestRate})</td>
                            <td style="padding: 12px; text-align: left;">${loan.borrowed}</td>
                            <td style="padding: 12px; text-align: left;">${loan.total}</td>
                            <td style="padding: 12px; text-align: left;">${loan.startDate}</td>
                            <td style="padding: 12px; text-align: left;">${loan.endDate}</td>
                            <td style="padding: 12px; text-align: left;">${loan.repaid}</td>
                            <td style="padding: 12px; text-align: left;">
                                <span style="display: inline-block; padding: 4px 8px; border-radius: 9999px; font-size: 12px; font-weight: 500;
                                    ${loan.status === 'Active' ? 'background-color: #d1fae5; color: #065f46;' :
                                    loan.status === 'Defaulted' ? 'background-color: #fee2e2; color: #991b1b;' :
                                    loan.status === 'Repaid' ? 'background-color: #d1fae5; color: #065f46;' :
                                    'background-color: #f3f4f6; color: #1f2937;'}">
                                    ${loan.status}
                                </span>
                            </td>
                    ...
                    printWindow.document.write(`
                        ...
                        <body>
                            ...
                            <div class="header">
                                <div>Generated on: ${new Date().toLocaleDateString()} ${new Date().toLocaleTimeString()}</div>
                                <div>Filters: Frequency - ${this.frequencyFilter}, Status - ${this.statusFilter}</div>
                                <div>Showing ${this.startEntry} to ${this.endEntry} of ${this.filteredTransactions.length} transactions</div>
                            </div>
        ...
        document.addEventListener('alpine:init', () => {
            ...
            // Register the loan transactions table component
            Alpine.data('loanTransactionsTable', loanTransactionsTable);
